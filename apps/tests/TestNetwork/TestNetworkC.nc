...
module TestNetworkC {
  ...
  //uses interface DisseminationValue<int> as DisseminationPeriod;
  //uses interface DisseminationUpdate<int> as DisseminationU;
  //uses interface Send;
  //uses interface Leds;
...
/*
  ...
  void sendMessage() {
    ...
    msg->seqno = seqno;
    msg->data = SIG* (1000*(i++) + TOS_NODE_ID);
    msg->parent = parent;
    msg->hopcount = 0;
    msg->metric = metric;
...
call Timer.startOneShot(5000*TOS_NODE_ID);
...
if (TOS_NODE_ID == 1) {
    ...
    SIG = SIG * -1;
    //call DisseminationU.change(&SIG);
...
/*
  event void DisseminationPeriod.changed() {
    const int* newVal = call DisseminationPeriod.get();
    SIG = *newVal;
...
/*
  ...
  event message_t* Receive.receive(message_t* msg, void* payload, uint8_t len) 
  {
    ...
    pay = (TestNetworkMsg*)call Send.getPayload(msg, sizeof(TestNetworkMsg));

    ORIG = call CollectionPacket.getOrigin(msg);
    DATA = pay->data;
